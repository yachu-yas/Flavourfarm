...
class User_Profile(Base_Model):
    for_user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='shopping_cart_profiles')
    def cart_items_count(self):
        count = Shopping_Cart_Items.objects.filter(for_cart__for_user = self.for_user, for_cart__payment_status = False).count()
        return count
